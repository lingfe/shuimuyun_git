...
CREATE TABLE `feedbacktag` (
  `feedbackId` varchar(100) NOT NULL COMMENT '意见反馈id',
  `kuaikeId` varchar(100) NOT NULL COMMENT '快客id',
  `feedbackDate` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '反馈时间',
  `status` int(11) DEFAULT NULL COMMENT '反馈状态:0未查看，1已查看,3，未处理，4已处理',
  `remark` text COMMENT '反馈内容',
...
CREATE TABLE `order_table` (
  ...
  `time_start` date DEFAULT NULL COMMENT '交易起始时间',
  `time_expire` date DEFAULT NULL COMMENT '交易结束时间',
  `urlCode` varchar(32) DEFAULT NULL COMMENT '二维码链接',
  `time_end` date DEFAULT NULL COMMENT '支付完成时间',
...
CREATE TABLE `xiaordertab` (
  ...
  `quhouDate` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '取货时间',
  ...
  `okDate` timestamp NULL DEFAULT '0000-00-00 00:00:00' COMMENT '到达时间',
  `shopName` varchar(64) DEFAULT NULL COMMENT '商品名称',
